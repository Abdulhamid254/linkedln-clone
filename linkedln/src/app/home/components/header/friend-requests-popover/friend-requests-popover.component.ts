...
import {
  FriendRequest,
  FriendRequestWithImagePath,
} from 'src/app/home/models/FriendRequest';
...
export class FriendRequestsPopoverComponent implements OnInit {
        ...
        if (friendRequest && creatorId) {
          this.connectionProfileService
            ...
            .pipe(
              take(1),
              tap((user: User) => {
                // adding a dynamic image property to friendRequest
                (friendRequest as FriendRequestWithImagePath).imagePath =
  ...
  async respondToFriendRequest(
    ...
    const unhandledFriendRequests: FriendRequest[] | undefined =
